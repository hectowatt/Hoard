...
// トップページに並ぶメモコンポーネント
export default function TrashNote({ id, title, content, label_id, createdate, updatedate, onRestore, onDelete }: trashNoteProps) {
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:4000/api/notes/trash/${id}`, {
                method: "DELETE",
                headers: {
